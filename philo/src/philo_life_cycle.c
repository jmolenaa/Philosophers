...
/*   Updated: 2023/08/18 10:33:49 by jmolenaa      ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
void	eat(t_philo *philo)
{
	...
	philo->time_of_last_eat = get_time_ms() - philo->data_struct->start_of_sim;
	pthread_mutex_unlock(&philo->data_struct->death);
	better_usleep(philo->data_struct->time_to_eat * 1000, philo->data_struct);
	philo->times_eaten++;
	pthread_mutex_lock(&philo->data_struct->death);
...
static void	_sleep(t_philo *philo)
{
	...
	better_usleep(philo->data_struct->time_to_sleep * 1000, philo->data_struct);
...
void	*philo_life_cycle(void *philo)
{
	...
	if (((t_philo *)philo)->id % 2 == 0)
		better_usleep(((t_philo *)philo)->data_struct->time_to_eat * 100, ((t_philo *)philo)->data_struct);
